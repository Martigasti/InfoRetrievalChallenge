"""
Wider pool + more body chunks on top of bge_weighted_rrf.py.

Two targeted changes vs the baseline:
  - RETRIEVAL_TOP_K 200 -> 300: more candidates enter RRF, higher recall ceiling
  - BODY_CHUNKS 3 -> 6: more document surface for both BGE and BM25 to match on

Everything else (models, weights, grid search) is identical to bge_weighted_rrf.py.
"""
...
def main():
    ...
    print(f"\n--- Wider RRF (pool={RETRIEVAL_TOP_K}, chunks={BODY_CHUNKS}, bm25={best_cfg}) ---")
    ...
    print(f"\nSaved -> {zip_path}  [pool={RETRIEVAL_TOP_K}, chunks={BODY_CHUNKS}, bm25_w={best_cfg}]")
