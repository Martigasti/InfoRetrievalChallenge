"""
3-way Weighted RRF: SPECTER2 + Snowflake-arctic-embed-m-v1.5 + BM25.

Based on bge_weighted_rrf.py, replacing BGE-large with Snowflake Arctic:
- #5 on MTEB academic retrieval (53.06 vs BGE's ~49)
- 109M params, 100% zero-shot
- Weighted RRF with BM25 weight grid search
"""
...
def main():
    ...
    arctic_model = SentenceTransformer(ARCTIC_MODEL_NAME, device=device)
    print("Encoding corpus with Snowflake Arctic...")
    arctic_corpus_embs = arctic_model.encode(corpus_enriched,
    ...
    for bm25_w in bm25_weights:
        fused = weighted_rrf_fuse(
            [(specter_ranking, 1.0), (arctic_ranking, 1.0), (bm25_ranking, bm25_w)],
    ...
    fused_best = weighted_rrf_fuse(
        [(specter_ranking, 1.0), (arctic_ranking, 1.0), (bm25_ranking, best_cfg)],
        k=RRF_K, top_k=FINAL_TOP_K
    )
    print(f"\n--- Weighted RRF (specter=1.0, arctic=1.0, bm25={best_cfg}) ---")
    ...
    ho_fused = weighted_rrf_fuse(
        [(ho_specter, 1.0), (ho_arctic, 1.0), (ho_bm25, best_cfg)],
        k=RRF_K, top_k=FINAL_TOP_K
    )
