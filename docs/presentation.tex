...
\vspace{0.6em}
\begin{itemize}
    \item Not topical similarity --- \emph{citation proximity}:``would paper A cite paper B?''
    \item Each query paper has a title, abstract, body text, and metadata
    \item Multi-domain corpus: Biology, Medicine, Computer Science, Physics, Chemistry\ldots
...
\begin{itemize}
    ...
    \item \textbf{Public queries}: 100 papers with ground-truth citations for local evaluation
...
% ═══════════════════════════════════════════════════════════════
% SLIDE 5 — TF-IDF Baseline
% ═══════════════════════════════════════════════════════════════
\begin{frame}{Step 0a: TF-IDF Baseline}
\textbf{Setup.} Classical sparse lexical baseline.\\
Input: \texttt{title + abstract}, cosine similarity on TF-IDF vectors.

\vspace{0.8em}
\begin{center}
\Large NDCG@10 $\approx$ \textbf{0.484}
\end{center}

\vspace{0.8em}
\textbf{Observation:}
\begin{itemize}
    \item Recall@100 already at 0.75 --- most relevant papers \emph{are} findable
          by lexical overlap alone
    \item But NDCG@10 is low: \emph{ranking} quality is the weak link, not recall
    \item Sets the bar every neural method must clear
\end{itemize}
\end{frame}

% ═══════════════════════════════════════════════════════════════
% SLIDE 6 — Dense Baseline
% ═══════════════════════════════════════════════════════════════
\begin{frame}{Step 0b: Dense Baseline}
...
\begin{axis}[
    ...
    xmin=-0.5, xmax=6.5,
    ymin=0.462, ymax=0.602,
    xtick={0,1,2,3,4,5,6},
    xticklabels={TF-IDF, Dense, SPECTER2, {+BM25}, {+Chunks}, {+BGE}, {+Wider}},
    xticklabel style={rotate=30, anchor=north east, font=\footnotesize},
    ytick={0.48,0.50,0.52,0.54,0.56,0.58,0.60},
    ...
    % Main series
    \addplot[blue!70!black, mark=*, mark size=2.5pt, thick, line width=1.2pt]
        coordinates {
            (0, 0.484)
            (1, 0.500)
            (2, 0.540)
    ...
    % Score labels — alternating above/below to avoid overlap in tight cluster
    \node[above=5pt, font=\scriptsize\bfseries] at (axis cs:0, 0.484) {0.484};
    \node[above=5pt, font=\scriptsize\bfseries] at (axis cs:1, 0.500) {0.500};
    \node[below=5pt, font=\scriptsize\bfseries] at (axis cs:2, 0.540) {0.540};
    \node[above=5pt, font=\scriptsize\bfseries] at (axis cs:3, 0.568) {0.568};
    \node[below=5pt, font=\scriptsize\bfseries] at (axis cs:4, 0.577) {0.577};
    \node[above=5pt, font=\scriptsize\bfseries] at (axis cs:5, 0.585) {0.585};
    \node[below=5pt, font=\scriptsize\bfseries] at (axis cs:6, 0.587) {0.587};
\end{axis}
\end{tikzpicture}
\end{frame}
